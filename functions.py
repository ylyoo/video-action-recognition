...
#data_path = "./jpegs_256/"
data_path = "/home/ubuntu/Project_video/jpegs_256"
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, x_RNN):
        ...
        RNN_out1, (h_n1, h_c1) = self.LSTM1(x_RNN, None)
        RNN_out2, (h_n2, h_c2) = self.LSTM2(RNN_out1, None)
        RNN_out3, (h_n3, h_c3) = self.LSTM3(RNN_out2, None)
        ...
        # FC layers
        x = self.fc1(RNN_out3[:, -1, :])   # choose RNN_out at the last time step
        x = F.relu(x)
        x = F.dropout(x, p=self.drop_p, training=self.training)
        x = self.fc2(x)
