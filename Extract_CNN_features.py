...
output_features = CNN_features([cnn_encoder, rnn_decoder], device, all_data_loader)
df = pd.DataFrame(data={'filename': fnames, 'y': cat2labels(le, all_y_list), 'features': output_features}) #이걸로 하면 됨.
df.to_pickle("./UCF101_CNN_features"+str(selected_epoch)+".pth")  # save pandas dataframe
# pd.read_pickle("./all_videos_prediction.pkl")
print('CNN feature extraction finished!')
